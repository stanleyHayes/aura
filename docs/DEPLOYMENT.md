...
1. **New + → Blueprint**, pick the repo. Render reads [`render.yaml`](../render.yaml)
   and provisions the whole backend in one shot: **`aura-postgres`** (managed Postgres,
   **PG 18**, `basic-256mb` ≈ $6/mo — Blueprint DBs can't be free) and **`aura-api`**
   (free Docker web service).
2. **Apply**, then open **aura-api → Environment** and set the `sync: false` secrets
   from `production.env`:
   - `JWT_SIGNING_KEY`
   - `MFA_ENCRYPTION_KEY`
   - `MAIL_PASSWORD`  (your Resend API key, `re_…`)
   - `CLOUDINARY_CLOUD_NAME`, `CLOUDINARY_API_KEY`, `CLOUDINARY_API_SECRET`
   (`DATABASE_URL` is wired automatically from the managed DB; `CORS_ALLOWED_ORIGINS`,
   the Resend host/port/user, and `MAIL_FROM=onboarding@resend.dev` are already in the blueprint.)
   > Want $0? Create a free Postgres in the dashboard, delete the `databases:` block
   > from `render.yaml`, and set `DATABASE_URL` to `sync: false` (paste its Internal URL).
